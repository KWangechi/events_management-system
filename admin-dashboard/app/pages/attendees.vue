...
const { data, status } = await useFetch<Attendee[]>('/api/attendees', {
  lazy: true
})

function getRowItems(row: Row<Attendee>) {
  return [
    {
      type: 'label',
      label: 'Actions'
    ...
    },
    {
      ...
      onSelect() {
        toast.add({
          ...
          description: 'The attendee has been deleted.'
        })
      }
    }
  ]
...
const columns: TableColumn<Attendee>[] = [
  ...
  },
  {
    ...
    filterFn: 'equals',
    cell: ({ row }) => {
      // Assign a random color to the event title
